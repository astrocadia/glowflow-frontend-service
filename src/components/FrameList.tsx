...
const FrameList: FunctionComponent<FrameListProps> = ({ savedFrames, selectedFrameIndex, displayFrame, deleteFrame, onReorder }) => {
            ...
            {savedFrames.map((frameData, index) => (
                <div className={`frame-preview ${index === selectedFrameIndex ? 'selected-frame' : ''}`} key={index}>
                    ...
                    <div className="button-container-frame">
                        <button onClick={() => deleteFrame(index)} disabled={savedFrames.length < 2}>X</button>
                        <button onClick={() => moveUp(index)} disabled={index === 0}>↑</button>
                        <button onClick={() => moveDown(index)} disabled={index === savedFrames.length - 1}>↓</button>
                    </div>
                </div>
            ))}
        </div>
